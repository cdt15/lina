...
def test_fit_lina():
    ...
    noise_ratios = 0.1
    n_features = 10
    n_samples, n_features_latent, n_edges, graph_type, sem_type = (
        1000,
        5,
        5,
        "ER",
...
def test_fit_mdlina():
    ...
    B_true = ut.simulate_dag(
        n_features_latent, n_edges, graph_type
    )  # skeleton btw. latent factors
    ...
    for j in range(n_features):
        ...
        G_nor1[j, :] = G1[j, :] / np.sqrt(
            np.square(np.sum(G1[j, :])) + np.square(noise_ratios)
        )
    X1 = (
        G_nor1 @ f_nor1.T + noise_ratios * e
    )  # "the noise ratio e is small or n_features is large"
    X1 = X1.T
